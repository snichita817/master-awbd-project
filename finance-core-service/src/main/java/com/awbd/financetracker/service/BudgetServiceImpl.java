...
public class BudgetServiceImpl implements BudgetService {
    ...
    public Budget createBudget(Long categoryId, Budget budget) {
        ...
        for (Subscription subscription : category.getSubscriptions()) {
            budget.setCurrentSpending(budget.getCurrentSpending().add(toMonthlyCost(subscription.getPrice(), subscription.getBillingFrequency())));
        }
    ...
    @Override
    @Transactional(readOnly = true)
    public List<Budget> getBudgetsByOwnerUserId(Long ownerUserId) {
        return budgetRepository.findByOwnerUserId(ownerUserId);
    }
    ...
    public Budget updateBudget(Long id, Budget updatedBudget) {
        ...
        Budget saved = budgetRepository.save(budget);
        log.info("Budget updated: id={}, maxLimit={}", saved.getId(), saved.getMaxLimit());
        return saved;
    }
    ...
    public void addSubscriptionToBudget(Category category, BigDecimal price, BillingFrequency frequency) {
        ...
        budgetRepository.findByCategoryId(category.getId()).ifPresent(budget -> {
            budget.setCurrentSpending(budget.getCurrentSpending().add(toMonthlyCost(price, frequency)));
    ...
    public void removeSubscriptionFromBudget(Category category, BigDecimal price, BillingFrequency frequency) {
        ...
        budgetRepository.findByCategoryId(category.getId()).ifPresent(budget -> {
            BigDecimal newSpending = budget.getCurrentSpending().subtract(toMonthlyCost(price, frequency));
